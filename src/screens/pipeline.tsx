...
import { DEALS, STAGES } from '../lib/data';
  ...
  return (
    <div className={`pipe-card${d.hot ? ' hot' : ''}${d.warm ? ' warm' : ''}`}>
      ...
      {menuEnabled && menuOpen && (
        <div role="menu" tabIndex={-1} className="pipe-card-menu" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="tags">
        {d.tags.map(t => <span key={t} className={`chip ${t === 'BAFO' || t === 'EXPIRED' ? 'accent' : t === 'PCS' ? 'olive' : t === 'CORP' || t === 'F500' ? 'info' : ''}`}>{t}</span>)}
      </div>
      <div className="foot">
        <span>{d.branch}</span>
...
export function PipelineKanban() {
  ...
  const moveDeal = React.useCallback(async (id: string, to: StageKey) => {
    ...
    if (!canTransition(from, to)) {
      track({
        ...
        props: { dealId: id, from, to, ms: 0, code: 'stage_transition_invalid', requestId: 'local' },
      });
    ...
    const local = await runTransition(eventLog, { id: prev.dealId, from, to, by: repId('rep_mhall'), role: 'rep', opKey });
    if (!local.ok) {
      track({
        name: 'pipeline.card_moved_failed',
        props: { dealId: id, from, to, ms: Math.round(performance.now() - t0), code: local.code, requestId: 'local' },
      });
      console.warn(`[pipeline] local commit refused: ${local.code} — ${local.message}`);
      return;
    }

    setDeals((ds) => ds.map((d) => (d.dealId === id ? { ...d, stage: to } : d)));
    ...
    if (!res.ok) {
      // Rollback both the UI and the log (compensating event).
      setDeals((ds) => ds.map((d) => (d.dealId === id ? { ...d, stage: from } : d)));
      void runTransition(eventLog, { id: prev.dealId, from: to, to: from, by: repId('rep_mhall'), role: 'rep', reason: `rolled back: ${res.error.code}` });
      track({
        name: 'pipeline.card_moved_failed',
        props: { dealId: id, from, to, ms, code: res.error.code, requestId: res.requestId },
      });
      console.warn(`[pipeline] move rolled back: ${res.error.code} — ${res.error.message} (req_id=${res.requestId})`);
      return;
    }
    track({ name: 'pipeline.card_moved_confirmed', props: { dealId: id, from, to, ms, requestId: res.requestId } });
  }, [deals]);

  const onDragStart = (e: React.DragEvent, id: string) => { setDragId(id); e.dataTransfer.effectAllowed = 'move'; };
  const onDragOver = (e: React.DragEvent, k: StageKey) => { e.preventDefault(); setOverStage(k); };
          ...
          >
            <div className="kan-col-head">
              <div>
                ...
                <div className="val">{count} · {formatMoneyShort(s.value)}</div>
              </div>
...
export function PipelineTimeline() {
  ...
  const rows: TimelineRow[] = [
    { n: 'SSgt. M. Alvarez',         i: 'LD-40218', bars: [{ s: 2, d: 3, cls: 'hot',  label: 'PCS · Campbell → JBLM',     amt: '$2,140' }] },
    { n: 'MELS Corporate Mobility',  i: 'ACC-1188', bars: [{ s: 0, d: 4, cls: 'hot',  label: 'BAFO window',                amt: '$740K MSA' }] },
    { n: 'CW3 Diane Park',           i: 'LD-40201', bars: [{ s: 3, d: 5, cls: '',     label: 'PCS · Rucker → Wainwright',  amt: '$8,300' }] },
    { n: 'Lt. Col. E. Oduya',        i: 'LD-40176', bars: [{ s: 4, d: 6, cls: 'info', label: 'OCONUS · Ramstein → WPAFB',  amt: '$6,410' }] },
    { n: 'Brightwell Energy Inc.',   i: 'LD-40268', bars: [{ s: 1, d: 4, cls: 'info', label: 'Annual contract scoping',    amt: '$220K' }] },
    { n: 'Capt. Julian Soto',        i: 'LD-40276', bars: [{ s: 5, d: 2, cls: '',     label: 'PCS · Coronado → Norfolk',   amt: '$3,150' }] },
    { n: 'Atlas Federal (GSA)',      i: 'LD-40149', bars: [{ s: 6, d: 4, cls: 'info', label: 'GSA · DOE task order',       amt: '$1.2M' }] },
    { n: 'Nordlight Capital',        i: 'LD-40108', bars: [{ s: 0, d: 2, cls: 'hot',  label: 'White-glove · exec',         amt: '$38K' }] },
    { n: 'MSgt. K. Vargas',          i: 'LD-40271', bars: [{ s: 8, d: 4, cls: '',     label: 'OCONUS · Eglin → Yokota',    amt: '$6,920' }] },
  ];
  return (
      ...
      {rows.map(r => (
        <div key={r.i} className="timeline-row">
          ...
          <div className="timeline-track">
            ...
            {r.bars.map((b, bi) => (
              <div key={bi} className={`tl-bar ${b.cls}`} style={{left: `${(b.s/12)*100}%`, width: `${(b.d/12)*100}%`}}>
...
const STAGE_LABEL: Record<StageKey, string> = {
  inbound: 'Inbound', qualify: 'Qualifying', scoping: 'Scoping', quote: 'Quoted', verbal: 'Verbal', won: 'Won',
};
        ...
        <tbody>
          {rows.map(d => (
            <tr key={d.dealId}>
              ...
              <td>
                <span className={`tag-rank ${d.tags.includes('PCS') ? 'pcs' : d.tags.includes('CORP') || d.tags.includes('F500') ? 'corp' : 'indiv'}`}>
                  {d.tags.includes('PCS') ? 'PCS' : d.tags.includes('CORP') || d.tags.includes('F500') ? 'CORP' : d.tags.includes('GSA') ? 'GSA' : 'INDIV'}
                </span>
              </td>
              ...
              <td>
                {d.hot ? <span className="c-accent">Due this week</span> :
                 d.warm ? <span className="c-muted">Re-quote needed</span> :
    ...
    <div className="pipeline">
      <div className="pane-head">
        <div className="pane-title">
          ...
          <div className="sub">One view of every deal in flight. Hot markers on cards mean the promise clock is running — not that the deal is big.</div>
        </div>
        <div className="row-gap-6">
          <button type="button" className="btn"><Filter className="ic-sm" aria-hidden="true" />Filter</button>
          <button type="button" className="btn"><Download className="ic-sm" aria-hidden="true" />Export</button>
          <button type="button" className="btn primary"><Plus className="ic-sm" aria-hidden="true" />New deal <kbd>N</kbd></button>
        </div>
      </div>
      <div className="controls">
        ...
        <div className="stats">
          <span><strong>94</strong> open</span>
          <span><strong>$1.67M</strong> weighted</span>
          <span><strong>34%</strong> win rate · 90d</span>
        </div>
      </div>
