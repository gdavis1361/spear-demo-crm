...
// Manager-role data — team queue, team pond. Also the "honest note" drafter.
    ...
    <div className="mgr-today">
      <div className="today-hero fade-up">
        <div>
          ...
          <h1>Two reps are at risk today. <em>One promise is overdue.</em></h1>
        </div>
        <div className="hero-right">
          <div className="hero-stat">
            <div className="s"><div className="n">6</div><div className="l">On deck</div></div>
            <div className="s"><div className="n">2</div><div className="l">At risk</div></div>
            <div className="s"><div className="n">1</div><div className="l">Overdue</div></div>
          </div>
        </div>
      </div>

      <div className="mgr-section-head">
        <h3>Team roster <span className="count">· sorted by risk, then by pod</span></h3>
        <div className="why">The manager view is not a leaderboard. It is a <strong className="sans-normal">"is anyone stuck?"</strong> view. Red means intervene.</div>
      </div>

      <div className="mgr-list">
        {[...MANAGER_TODAY].sort((a,b) => (b.risk === 'high' ? 1 : 0) - (a.risk === 'high' ? 1 : 0)).map(r => (
          <div key={r.rep} className={`mgr-row risk-${r.risk}`}>
            <div className="mgr-rep">
              <div className="ini">{r.rep.split(' ').map(x => x[0]).join('').replace('.','')}</div>
              <div>
            ...
            <div className="mgr-state">
              ...
              <div className="now">Now · <em>{r.now}</em></div>
            </div>
            <div className="mgr-promise">
              <div className="k">Promise watch</div>
              <div className={`v${r.promise.includes('overdue') ? ' overdue' : ''}`}>{r.promise}</div>
            </div>
            <div className="mgr-actions">
              {r.risk === 'high' ? (
                <button type="button" className="btn primary"><MessageSquare className="ic-sm" aria-hidden="true" />Check in</button>
              ) : (
                <button type="button" className="btn"><Eye className="ic-sm" aria-hidden="true" />Open queue</button>
              )}
            </div>
          </div>
        ))}
      ...
      <div className="mgr-note">
        ...
        <div className="body">
          A rep's Today answers <em>"who should I call?"</em>. A manager's Today answers <em>"is anyone stuck?"</em> — those are different questions and they deserve different screens. We resisted making this a dashboard with KPIs. The KPIs are on Pond.
        </div>
      </div>
    </div>
  );
    ...
    <div className="pond">
      <div className="hero">
        <div>
          ...
          <h1>Team is <em>carrying the pod</em> — but the pod is carrying <em>one person</em>.</h1>
        </div>
      ...
      <div className="pond-grid">
        <div className="pond-card pond-span-8">
          ...
          <div className="headline">M. Hall is producing <em>48% of pod MRR</em>. If she takes PTO the week of May 06, we have a problem.</div>
          <div className="body">Two deals in her pipeline (MELS · Nordlight) are irreplaceable by another rep at her current stage. We should pair K. Okonkwo on MELS this week — not to shadow, but to build a real second relationship with Rene Odita.</div>
        ...
        <div className="pond-card pond-span-4">
          ...
          <div className="big">4 / 6<em> healthy</em></div>
          <div className="body body-sm-muted">Brennan · no activity 2h+. Hemming · queue stale. Both get a slack from me this morning, not a ticket.</div>
          <div className="foot">Signals · last 24h</div>
        </div>

        <div className="pond-card pond-span-6">
          <div className="eyebrow">Team velocity</div>
          <div className="headline">First-touch time is <em>down to 28 minutes</em> · median, last 30 days.</div>
          <div className="body">Two months ago it was 54. The focus-mode rollout closed the gap. We should not celebrate this in a deck — we should make sure it does not regress when we add quote-builder training next month.</div>
          <div className="foot">Source · touch timestamps · n=214</div>
        </div>
        <div className="pond-card pond-span-6">
          <div className="eyebrow">Where the pod is losing</div>
          <div className="headline">OCONUS partial-pack · <em>14% win rate</em>. Corporate-adjacent SMB is worse.</div>
          <div className="body">This is a training gap, not a product gap. Reps are discounting at the first sign of resistance. We'll pair D. Laurent with Brennan on two SMB deals this week to model the honest-price conversation.</div>
...
export function HonestDraft({ open, onClose, onInsert }: HonestDraftProps) {
  ...
  return (
    <div className="draft-overlay" onClick={onClose} role="presentation">
      <div className="draft" onClick={e => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="draft-title">
        <div className="draft-head">
          <div>
            <div className="eyebrow">Honest note · drafter</div>
            <h3 id="draft-title">What we tell Rachel Alvarez, in our own words</h3>
            <div className="draft-sub">This is the paragraph the shipper sees before the price. It is the paragraph the rep reads out loud on the call. It is the thing we do not outsource to marketing.</div>
          </div>
          <button type="button" className="icon-btn" onClick={onClose} aria-label="Close"><X className="ic" aria-hidden="true" /></button>
        </div>
          ...
          <div className="draft-left">
            ...
            <div className="draft-tmpls">
              {HONEST_TEMPLATES.map(t => (
                <button type="button" key={t.id} aria-pressed={tmpl === t.id} className={`draft-tmpl${tmpl === t.id ? ' on' : ''}`} onClick={() => setTmpl(t.id)}>
          ...
          <div className="draft-right">
            ...
            <div className="draft-checks">
              <div className={`chk ${!/!/.test(text) ? 'ok' : 'fail'}`}>
                {!/!/.test(text) ? <Check className="ic-sm" aria-hidden="true" /> : <X className="ic-sm" aria-hidden="true" />}
                <span>No exclamation points</span>
              </div>
              <div className={`chk ${!/(excited|thrilled|delighted)/i.test(text) ? 'ok' : 'fail'}`}>
                {!/(excited|thrilled|delighted)/i.test(text) ? <Check className="ic-sm" aria-hidden="true" /> : <X className="ic-sm" aria-hidden="true" />}
                <span>No sales adjectives</span>
              </div>
              <div className={`chk ${/dispatcher/i.test(text) ? 'ok' : 'fail'}`}>
                {/dispatcher/i.test(text) ? <Check className="ic-sm" aria-hidden="true" /> : <X className="ic-sm" aria-hidden="true" />}
                <span>Names a dispatcher</span>
              </div>
              <div className={`chk ${wc >= 30 && wc <= 80 ? 'ok' : 'fail'}`}>
                {wc >= 30 && wc <= 80 ? <Check className="ic-sm" aria-hidden="true" /> : <X className="ic-sm" aria-hidden="true" />}
        ...
        <div className="draft-foot">
          <div className="draft-foot-note">The paragraph updates in the live shipper preview the moment you insert it.</div>
          <div className="draft-foot-actions">
            ...
            >
              <Check className="ic-sm" aria-hidden="true" />Insert
            </button>
          </div>
        </div>
      </div>
...
const DEALS_LIST = [
  { id: 'ACC-1188 / MSA-2025-041', title: 'Annual MSA · 2026 renewal', stage: 'BAFO', value: '$740K', close: 'Wed', cls: 'hot', note: 'Line-haul narrative for 5 ATL lanes is the unblocker.' },
  { id: 'LD-40021', title: 'Maj. L. Okafor · Atlanta → Benning', stage: 'Won', value: '$3,220', close: 'Apr 08 · signed', cls: 'won', note: 'Claims on humidor closed 36h — Katherine cited in QBR.' },
  { id: 'LD-39740', title: 'Eng. VP relocation · ATL → Boulder', stage: 'Won', value: '$12,410', close: 'Feb 22 · signed', cls: 'won', note: 'Executive white-glove. Shipper = K. Ruiz\'s peer.' },
  { id: 'LD-39188', title: '4-family Q1 batch · ATL metro', stage: 'Won', value: '$22,900', close: 'Jan 18 · signed', cls: 'won', note: 'MELS trialled us against Weichert · we won 3/4.' },
  { id: 'LD-38012', title: 'Pilot · 2 relos', stage: 'Won', value: '$6,400', close: 'Oct 2024', cls: 'won', note: 'First deals. Our foot in the door.' },
];
const DOCS_LIST = [
  { n: 'MSA-2025-041 · draft v3.pdf',             by: 'M. Hall · today 06:14',       size: '142 KB', tag: 'MSA' },
  { n: 'Weichert proposal · fwd Apr 20.pdf',      by: 'K. Ruiz · Apr 20 · 18:04',    size: '1.1 MB', tag: 'COMPETITOR' },
  { n: 'QBR deck · Atlanta HQ · Apr 14.pptx',     by: 'M. Hall · Apr 14 · 17:40',    size: '3.8 MB', tag: 'QBR' },
  { n: 'Claims case · Oduya · Mar 06.pdf',        by: 'Claims · Mar 06',             size: '88 KB',  tag: 'CLAIM' },
  { n: 'Line-haul rate card · ATL lanes.xlsx',    by: 'S. Brennan · Apr 19',         size: '64 KB',  tag: 'PRICING' },
  { n: 'Partner coverage map · Q2.pdf',           by: 'N. Fogerty · Apr 12',         size: '240 KB', tag: 'OPS' },
];
const ACTIVITY_LIST = [
  { t: '08:14 CDT', k: 'Message',  who: 'M. Hall',      body: 'Reading now. I\'ll have a numbered response…',         tag: 'outbound' },
  { t: '08:09 CDT', k: 'Message',  who: 'K. Ruiz',      body: 'Forwarding Weichert\'s proposal. Line-haul pricing…', tag: 'inbound' },
  { t: 'Mon 17:40', k: 'Signal',   who: 'system',       body: 'Katherine opened "MSA draft v2" three times in 18 min — pattern: reviewing with someone.', tag: 'signal' },
  { t: 'Mon 14:22', k: 'File',     who: 'M. Hall',      body: 'Uploaded MSA-2025-041 draft v3.pdf',                   tag: 'file' },
  { t: 'Apr 17',    k: 'Meeting',  who: 'M. Hall',      body: 'Internal · BAFO strategy w/ S. Brennan (45 min)',      tag: 'internal' },
  { t: 'Apr 14',    k: 'Meeting',  who: 'K. Ruiz + 2',  body: 'Quarterly business review · Atlanta HQ (90 min)',      tag: 'meeting' },
  { t: 'Mar 06',    k: 'Claim',    who: 'Claims team',  body: 'Oduya claim · $1,200 humidor · paid in 36h',           tag: 'claim' },
];

export function AccountDeals() {
  return (
    <section>
      <h3 className="acct-section-title">All deals with MELS <span className="muted">· 5 total · $785K lifetime</span></h3>
      <div className="adl">
        {DEALS_LIST.map(d => (
...
export function AccountDocs() {
  return (
    <section>
      <h3 className="acct-section-title">Documents <span className="muted">· 6 · last updated today 06:14</span></h3>
      <div className="adoc">
        {DOCS_LIST.map(d => (
          <div key={d.n} className="adoc-row">
            ...
            <button type="button" className="btn ghost btn-compact">Open</button>
          </div>
        ))}
      </div>
    </section>
...
export function AccountActivity() {
  return (
    <section>
      <h3 className="acct-section-title">Activity <span className="muted">· all events · filtered: all</span></h3>
      <div className="aact">
