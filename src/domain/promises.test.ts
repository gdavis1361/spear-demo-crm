...
async function clearAllStores(): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    const tx = db.transaction(
      ['events', 'events_dlq', STORE_PROMISES, STORE_PROMISES_DLQ],
      'readwrite'
    );
...
describe('PromiseStore (row-level IDB)', () => {
  ...
  it('list() is sorted by dueAt ascending', async () => {
    await store.create({
      ...
      noun: { kind: 'person', id: 'p' },
      text: 'late',
      dueAt: instant('2026-04-21T20:00:00Z'),
      createdBy: me,
    });
    await store.create({
      id: 'pr_soon',
      noun: { kind: 'person', id: 'p' },
      text: 'soon',
      dueAt: instant('2026-04-21T14:00:00Z'),
      createdBy: me,
    });
    await store.create({
      id: 'pr_mid',
  ...
  it('broadcast channel is DB-scoped: a sibling channel on a different DB name is ignored', async () => {
    // A tab on `?seed=other-scenario` would open `spear:promises:<other-db>`.
    // Simulate that peer by constructing a BroadcastChannel with a non-matching
    // name and posting an upsert — the local store (on the test DB) must not
    // apply it. Pre-fix this leaked: both tabs used `spear:promises` flat, so
    // unrelated scenario DBs cross-polluted each other's caches.
    if (typeof BroadcastChannel === 'undefined') return;

    const updates: number[] = [];
    store.subscribe((ps) => updates.push(ps.length));
    const initialLen = store.list().length;

    const otherDbChannel = new BroadcastChannel('spear:promises:some-other-db');
    otherDbChannel.postMessage({
      kind: 'upsert',
      id: 'pr_leak',
      row: {
        id: 'pr_leak',
        noun: { kind: 'person', id: 'p' },
        text: 'should not appear in this DB',
        dueAt: { iso: '2026-04-21T16:00:00Z' },
        createdAt: { iso: '2026-04-21T13:00:00Z' },
        updatedAt: { iso: '2026-04-21T13:00:00Z' },
        createdBy: me,
        status: 'pending',
      },
    });

    // Yield so the (non-matching) channel's message has a chance to route
    // anywhere it shouldn't.
    await new Promise((r) => setTimeout(r, 10));

    expect(store.list().find((p) => p.id === 'pr_leak')).toBeUndefined();
    expect(store.list()).toHaveLength(initialLen);
    otherDbChannel.close();
  });
});
